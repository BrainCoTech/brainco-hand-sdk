#!/usr/bin/env python3
"""
Stark Revo3 (Revo3) Motor Control Demo - 21 DoF Dexterous Hand
...
async def demo_position_control(ctx, slave_id):
    ...
    # Read back
    positions = await ctx.revo3_get_all_motor_positions(slave_id)
    print(f"  Positions (first 5): {[f'{p:.1f}' for p in positions[:5]]}")

    # Batch: all motors
    print("  All motors -> 30 degrees")
    target = [30.0] * REVO3_MOTOR_COUNT
    await ctx.revo3_set_all_motor_positions(slave_id, target)
    await asyncio.sleep(1.0)

    # Open all
    print("  All motors -> 0 degrees")
    await ctx.revo3_set_all_motor_positions(slave_id, [0.0] * REVO3_MOTOR_COUNT)
    await asyncio.sleep(0.5)


async def demo_velocity_control(ctx, slave_id):
    """Single and batch motor velocity control"""
    print("\n=== Velocity Control ===")

    print("  Motor 0 velocity -> 100.0 RPM")
    await ctx.revo3_set_motor_velocity(slave_id, 0, 100.0)
    await asyncio.sleep(0.5)

    print("  All motors velocity -> 50.0 RPM")
    await ctx.revo3_set_all_motor_velocities(slave_id, [50.0] * REVO3_MOTOR_COUNT)
    await asyncio.sleep(0.5)

    # Stop
    print("  Stop all motors")
    await ctx.revo3_set_all_motor_velocities(slave_id, [0.0] * REVO3_MOTOR_COUNT)
...
async def demo_current_control(ctx, slave_id):
    ...
    await ctx.revo3_set_motor_current(slave_id, 0, 500.0)
    await asyncio.sleep(0.5)
...
async def demo_single_joint(ctx, slave_id):
    """Single joint control"""
    print("\n=== Single Joint Control ===")
    print("  Registers: 1000 (joint_id) + 1001 (mode) + 1002 (param)")

    # Position mode: joint 0 -> 45 degrees
    mode = sdk.V3ControlMode.Position
    print(f"  Joint 0: mode=Position({int(mode)}), param=45.0°")
    # Note: param is a float (degrees for Position, rpm for Speed, mA for Current)
    await ctx.revo3_single_joint_control(slave_id, 0, int(mode), 45.0)
    await asyncio.sleep(0.5)


async def demo_multi_joint(ctx, slave_id):
    """Multi-joint synchronous control"""
    print("\n=== Multi-Joint Synchronous Control ===")
    print("  Registers: 1010 (mode) + 1011~1031 (21 params)")

    mode = sdk.V3ControlMode.Position
    params = [30.0] * REVO3_MOTOR_COUNT  # 21 joints to 30.0°
    print(f"  All 21 joints: mode=Position, param=30.0°")
    await ctx.revo3_multi_joint_control(slave_id, int(mode), params)
    await asyncio.sleep(1.0)


async def demo_mit_control(ctx, slave_id):
...
async def demo_motor_params(ctx, slave_id):
...
async def demo_teaching_mode(ctx, slave_id):
    """Teaching mode demo"""
    print("\n=== Teaching Mode ===")

    print("  Entering teaching mode...")
    await ctx.revo3_set_teaching_mode(slave_id, True)
    await asyncio.sleep(1.0)

    print("  (In teaching mode, motors can be moved by hand)")
    print("  Exiting teaching mode...")
    await ctx.revo3_set_teaching_mode(slave_id, False)
    await asyncio.sleep(0.5)


async def demo_peripherals(ctx, slave_id):
...
async def demo_multi_mit(ctx, slave_id):
...
async def demo_batch_mit(ctx, slave_id):
...
async def demo_finger_control(ctx, slave_id):
    ...
    # Index finger (30.0°)
    print("  Index finger: mode=Position, params=[30.0, 30.0, 30.0, 30.0]°")
    await ctx.revo3_finger_control(slave_id, 1, mode, [30.0, 30.0, 30.0, 30.0])
    await asyncio.sleep(0.5)

    # Thumb (20.0°)
    print("  Thumb: mode=Position, params=[20.0, 20.0, 20.0, 20.0, 20.0]°")
    await ctx.revo3_thumb_control(slave_id, mode, [20.0, 20.0, 20.0, 20.0, 20.0])
    await asyncio.sleep(0.5)


async def demo_motor_temps(ctx, slave_id):
...
async def demo_motor_info(ctx, slave_id):
...
async def main():
    ...
    if not ctx.uses_revo3_motor_api(slave_id):
        ...
        print("Revo3 APIs may not work correctly on this device.")

    try:
        # Read-only demos first
        await demo_device_info(ctx, slave_id)
        await demo_motor_params(ctx, slave_id)
        await demo_motor_status(ctx, slave_id)
        await demo_motor_temps(ctx, slave_id)
        await demo_motor_info(ctx, slave_id)
        # Control demos
        await demo_position_control(ctx, slave_id)
        await demo_velocity_control(ctx, slave_id)
        await demo_current_control(ctx, slave_id)
        await demo_single_joint(ctx, slave_id)
        await demo_multi_joint(ctx, slave_id)
        await demo_mit_control(ctx, slave_id)
        await demo_multi_mit(ctx, slave_id)
        await demo_batch_mit(ctx, slave_id)
        await demo_finger_control(ctx, slave_id)
        await demo_teaching_mode(ctx, slave_id)
        await demo_peripherals(ctx, slave_id)
